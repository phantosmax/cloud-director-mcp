...
class VmwareCloudDirectorMCPServer {
        ...
        try {
            ...
            return {
                content: [
                    {
                        type: 'text',
                        text: `Found ${orgs.length} organizations:\n${orgs.map((org) => `- ${org.name} (ID: ${org.id})`).join('\n')}`,
                    },
                ],
            };
        }
    ...
    async handleGetVdcDetails(vdcId) {
        await this.ensureAuthenticated();
        try {
            if (!vdcId)
                throw new Error('VDC ID is required');
            // Get basic VDC info
            ...
            const networkProvider = capabilities.find((cap) => cap.name === 'networkProvider')?.value || 'Unknown';
            const supportedVdcGroups = capabilities.find((cap) => cap.name === 'vdcGroupTypes')?.value || [];
            const supportedNetworkProviders = capabilities.find((cap) => cap.name === 'vdcGroupNetworkProviderTypes')?.value || [];
            // Key capabilities
            ...
            const supportedFeatures = keyCapabilities
                .filter(capName => capabilities.find((cap) => cap.name === capName)?.value === true)
                .join(', ');
            return {
                content: [
                    {
        ...
        try {
            ...
            if (typeof response.data === 'string') {
                try {
                    ...
                    if (parsed.QueryResultRecords && parsed.QueryResultRecords.VAppRecord) {
                        vapps = parsed.QueryResultRecords.VAppRecord.map((record) => ({
                            name: record.$.name,
            ...
            return {
                content: [
                    {
                        type: 'text',
                        text: `Found ${vapps.length} vApps:\n${vapps.map((vapp) => `- ${vapp.name} (Status: ${vapp.status}, ID: ${vapp.href?.split('/').pop()})`).join('\n')}`,
                    },
                ],
            };
        }
    ...
    async handleListVms(vappId, status) {
        await this.ensureAuthenticated();
        try {
            const allVms = [];
            const searchResults = [];
            ...
            for (const queryType of queryTypes) {
                try {
                    ...
                    const filteredVms = items.filter((vm) => {
                    ...
                    searchResults.push(`${queryType.description}: Failed (${queryError.response?.status || 'error'})`);
                }
            }
            ...
            try {
                ...
                const filteredCloudVms = cloudVms.filter((vm) => {
                ...
                searchResults.push(`CloudAPI VMs: Failed (${cloudError.response?.status || 'error'})`);
            }
            // Remove duplicates based on VM name
            const uniqueVms = allVms.filter((vm, index, self) => index === self.findIndex(v => v.name === vm.name));
            let statusText = status ? ` with status ${status}` : '';
            let vappText = vappId ? ` in vApp ${vappId}` : '';
            const vmList = uniqueVms.map((vm) => `- ${vm.name} (Status: ${vm.status}, CPU: ${vm.numberOfCpus || vm.cpuCount || 'N/A'}, Memory: ${vm.memoryMB || 'N/A'}MB, VDC: ${vm.vdcName || vm.vdc?.name || 'N/A'}, Container: ${vm.containerName || vm.vapp?.name || 'Standalone'}, ID: ${vm.href?.split('/').pop() || vm.id})`).join('\n');
            return {
                content: [
                    {
                        type: 'text',
    ...
    async handleSearchVms(name, vdcName) {
        await this.ensureAuthenticated();
        try {
            const allVms = [];
            const searchResults = [];
    ...
    async handleVmPowerAction(vmId, action) {
        await this.ensureAuthenticated();
        try {
            if (!vmId)
                throw new Error('VM ID is required');
    ...
    async handleVappPowerAction(vappId, action) {
        await this.ensureAuthenticated();
        try {
            if (!vappId)
                throw new Error('vApp ID is required');
            if (!action)
                throw new Error('Action is required');
    ...
    async handleListNetworks(orgId) {
        await this.ensureAuthenticated();
        try {
            ...
            return {
                content: [
                    {
                        type: 'text',
                        text: `Found ${networks.length} organization VDC networks:\n${networks.map((network, index) => `${index + 1}. ${network.name} (Type: ${network.networkType}, VDC: ${network.orgVdc?.name || 'N/A'}, IPs Used: ${network.usedIpCount}/${network.totalIpCount}, Shared: ${network.shared})`).join('\n') || '(none)'}`,
                    },
                ],
            };
        }
    ...
    async handleGetResourceUsage(orgId) {
        await this.ensureAuthenticated();
        try {
    ...
    async handleListFirewallRules(vdcId) {
        await this.ensureAuthenticated();
        try {
            if (!vdcId)
                throw new Error('VDC ID is required');
            ...
            try {
                    ...
                    try {
                        ...
                        if (userRules.length > 0) {
                            ...
                            userRules.forEach((rule, index) => {
                                ...
                                results.push(`       Source Groups: ${rule.sourceFirewallGroups?.map((g) => g.name).join(', ') || 'Any'}`);
                                results.push(`       Source IPs: ${rule.sourceFirewallIpAddresses?.join(', ') || 'Any'}`);
                                results.push(`       Destination Groups: ${rule.destinationFirewallGroups?.map((g) => g.name).join(', ') || 'Any'}`);
                                results.push(`       Destination IPs: ${rule.destinationFirewallIpAddresses?.join(', ') || 'Any'}`);
                                results.push(`       Services: ${rule.applicationPortProfiles?.map((p) => p.name).join(', ') || 'Any'}`);
                        ...
                        results.push(`  Failed to get firewall rules for ${edgeGateway.name}: ${firewallError.response?.status || 'error'}`);
                    }
                }
            }
            catch (edgeError) {
                ...
                try {
                    ...
                    results.push(`Legacy API also failed: ${legacyError.response?.status || 'error'}`);
                }
            }
            return {
                content: [
    ...
    async handleListTasks(vdcId, hours) {
        await this.ensureAuthenticated();
        try {
            ...
            try {
                ...
                if (tasks.length === 0) {
                    ...
                    tasks.forEach((task, index) => {
                        ...
                        if (task.details)
                            results.push(`     Details: ${task.details}`);
                        results.push('');
                    });
                }
            }
            catch (cloudApiError) {
                results.push(`CloudAPI tasks query failed: ${cloudApiError.response?.status || 'error'}`);
                // Try legacy API
                try {
                    ...
                    if (typeof response.data === 'string') {
                        try {
                            ...
                            if (parsed.QueryResultRecords && parsed.QueryResultRecords.TaskRecord) {
                                tasks = parsed.QueryResultRecords.TaskRecord.map((record) => ({
                                    name: record.$.name,
                                    status: record.$.status,
                                ...
                                if (vdcId) {
                                    tasks = tasks.filter((task) => task.vdcName && task.vdcName.includes(vdcId));
                                }
                            }
                        }
                        catch (xmlError) {
                    ...
                    results.push(`Legacy API tasks query also failed: ${legacyError.response?.status || 'error'}`);
                }
            }
            return {
                content: [
    ...
    async handleVmConfigure(vmId, cpuCount, memoryMB) {
        await this.ensureAuthenticated();
        try {
            if (!vmId)
                throw new Error('VM ID is required');
            if (!cpuCount && !memoryMB)
                throw new Error('At least one configuration parameter (cpuCount or memoryMB) is required');
    ...
    async handleVmCreateSnapshot(vmId, snapshotName, snapshotDescription) {
        await this.ensureAuthenticated();
        try {
            if (!vmId)
                throw new Error('VM ID is required');
    ...
    async handleVmListSnapshots(vmId) {
        await this.ensureAuthenticated();
        try {
            if (!vmId)
                throw new Error('VM ID is required');
            ...
            return {
                content: [
                    {
                        type: 'text',
                        text: `Found ${snapshots.length} snapshots for VM ${vmId}:\n${snapshots.map((snapshot, index) => `${index + 1}. ${snapshot.name} (Created: ${snapshot.createdDate}, Size: ${snapshot.size || 'N/A'} MB, ID: ${snapshot.id})`).join('\n') || '(none)'}`,
                    },
                ],
            };
        }
    ...
    async handleVmRestoreSnapshot(vmId, snapshotId) {
        await this.ensureAuthenticated();
        try {
            if (!vmId)
                throw new Error('VM ID is required');
    ...
    async handleVmClone(vmId, newVmName, vdcId) {
        await this.ensureAuthenticated();
        try {
            if (!vmId)
                throw new Error('VM ID is required');
            if (!newVmName)
                throw new Error('New VM name is required');
    ...
    async handleListDatastores(vdcId) {
        await this.ensureAuthenticated();
        try {
            ...
            if (typeof response.data === 'string') {
                try {
                    ...
                    if (parsed.QueryResultRecords && parsed.QueryResultRecords.OrgVdcStorageProfileRecord) {
                        storageProfiles = parsed.QueryResultRecords.OrgVdcStorageProfileRecord.map((record) => ({
                            name: record.$.name,
            ...
            if (vdcId) {
                ...
                filteredProfiles = storageProfiles.filter((profile) => profile.vdcName === vdcName);
            }
            return {
                content: [
                    {
                        type: 'text',
                        text: `Found ${filteredProfiles.length} storage profiles:\n${filteredProfiles.map((profile, index) => `${index + 1}. ${profile.name} (Enabled: ${profile.enabled}, Default: ${profile.isDefault}, Used: ${profile.storageUsedMB || 0}MB / ${profile.storageLimitMB || 'Unlimited'}MB, VDC: ${profile.vdcName})`).join('\n') || '(none)'}`,
                    },
                ],
            };
        }
    ...
    async handleVmAddDisk(vmId, diskSizeMB, diskName) {
        await this.ensureAuthenticated();
        try {
            if (!vmId)
                throw new Error('VM ID is required');
        ...
        try {
            ...
            return {
                content: [
                    {
                        type: 'text',
                        text: `Found ${disks.length} disks for VM ${vmId}:\n${disks.map((disk, index) => `${index + 1}. ${disk.name || 'Unnamed'} (Size: ${disk.sizeMb} MB, Bus: ${disk.busType}:${disk.busNumber}:${disk.unitNumber}, ID: ${disk.id})`).join('\n') || '(none)'}`,
                    },
                ],
            };
        }
    ...
    async handleVmResizeDisk(vmId, diskId, diskSizeMB) {
        await this.ensureAuthenticated();
        try {
            if (!vmId)
                throw new Error('VM ID is required');
            ...
            if (!diskSizeMB)
                throw new Error('New disk size is required');
            const payload = {
                sizeMb: diskSizeMB
            };
    ...
    // Networking Handlers
    async handleCreateOrgNetwork(networkName, networkType, vdcId) {
        await this.ensureAuthenticated();
        try {
            if (!networkName)
    ...
    async handleListNatRules(vdcId) {
        await this.ensureAuthenticated();
        try {
            if (!vdcId)
                throw new Error('VDC ID is required');
    ...
    async handleListDhcpPools(vdcId) {
        await this.ensureAuthenticated();
        try {
            if (!vdcId)
                throw new Error('VDC ID is required');
    ...
    async handleListIpAllocations(vdcId) {
        await this.ensureAuthenticated();
        try {
            if (!vdcId)
                throw new Error('VDC ID is required');
    ...
    async handleListCatalogs(orgId) {
        await this.ensureAuthenticated();
        try {
            ...
            if (typeof response.data === 'string') {
                try {
                    ...
                    if (parsed.QueryResultRecords && parsed.QueryResultRecords.CatalogRecord) {
                        catalogs = parsed.QueryResultRecords.CatalogRecord.map((record) => ({
                            name: record.$.name,
                            description: record.$.description,
            ...
            return {
                content: [
                    {
                        type: 'text',
                        text: `Found ${catalogs.length} catalogs:\n${catalogs.map((catalog, index) => `${index + 1}. ${catalog.name} (Templates: ${catalog.numberOfVAppTemplates || 0}, Media: ${catalog.numberOfMedia || 0}, Published: ${catalog.isPublished}, Shared: ${catalog.isShared}, Org: ${catalog.orgName})`).join('\n') || '(none)'}`,
                    },
                ],
            };
        }
    ...
    async handleListCatalogItems(catalogName) {
        await this.ensureAuthenticated();
        try {
            if (!catalogName)
                ...
                try {
                    ...
                    if (parsed.QueryResultRecords && parsed.QueryResultRecords.CatalogItemRecord) {
                        catalogItems = parsed.QueryResultRecords.CatalogItemRecord
                            .filter((record) => record.$.catalogName === catalogName)
                            .map((record) => ({
                            name: record.$.name,
            ...
            return {
                content: [
                    {
                        type: 'text',
                        text: `Found ${catalogItems.length} items in catalog '${catalogName}':\n${catalogItems.map((item, index) => `${index + 1}. ${item.name} (Entity: ${item.entityName}, Type: ${item.entityType}, Created: ${item.creationDate}, Owner: ${item.ownerName})`).join('\n') || '(none)'}`,
                    },
                ],
            };
        }
    ...
    async handleGetTemplateDetails(templateName, catalogName) {
        await this.ensureAuthenticated();
        try {
            if (!templateName)
            ...
            const catalog = catalogs.find((cat) => cat.name === catalogName);
            if (!catalog) {
            ...
            const template = items.find((item) => item.name === templateName);
            if (!template) {
    ...
    async handleGetVmMetrics(vmId, metricType) {
        await this.ensureAuthenticated();
        try {
            if (!vmId)
                throw new Error('VM ID is required');
    ...
    async handleListEvents(hours, vdcId) {
        await this.ensureAuthenticated();
        try {
            const lookbackHours = hours || 24;
            const startTime = new Date(Date.now() - lookbackHours * 60 * 60 * 1000);
            ...
            try {
                ...
                if (typeof response.data === 'string') {
                    ...
                    if (parsed.QueryResultRecords && parsed.QueryResultRecords.EventRecord) {
                        events = parsed.QueryResultRecords.EventRecord.map((record) => ({
                ...
                return {
                    content: [
                        {
                            type: 'text',
                            text: `Found ${events.length} events in the last ${lookbackHours} hours:\n${events.map((event, index) => `${index + 1}. ${event.eventType} at ${event.timeStamp} by ${event.userName}: ${event.description}`).join('\n') || '(none)'}`,
                        },
                    ],
                };
            }
    ...
    async handleGetOrgHealth(orgId) {
        await this.ensureAuthenticated();
        try {
            ...
            const runningVapps = vapps.filter((vapp) => vapp.$.status === 'POWERED_ON').length;
            const stoppedVapps = vapps.filter((vapp) => vapp.$.status === 'POWERED_OFF').length;
            const readyVdcs = vdcs.filter((vdc) => vdc.status === 'READY').length;
            return {
                content: [
                    {
                        type: 'text',
    ...
    async handleGetResourceMetrics(vdcId, metricType) {
        await this.ensureAuthenticated();
        try {
            if (vdcId) {
                    ...
                    try {
                        ...
                        if (parsed.QueryResultRecords && parsed.QueryResultRecords.OrgVdcStorageProfileRecord) {
                            storageProfiles = parsed.QueryResultRecords.OrgVdcStorageProfileRecord
                                .filter((record) => record.$.vdcName === vdc.name);
                        }
                    }
                    catch (xmlError) {
                        console.error('Failed to parse storage profiles XML:', xmlError);
                ...
                const totalStorage = storageProfiles.reduce((sum, profile) => sum + parseInt(profile.$.storageLimitMB || '0'), 0);
                const usedStorage = storageProfiles.reduce((sum, profile) => sum + parseInt(profile.$.storageUsedMB || '0'), 0);
                return {
                    content: [
                        {
                            type: 'text',
                    ...
                    content: [
                        {
                            ...
                            text: `Organization Resource Metrics${metricType ? ` (${metricType})` : ''}:\n\n` +
                                ...
                                `Ready VDCs: ${vdcs.filter((vdc) => vdc.status === 'READY').length}\n` +
                                `Total Networks: ${networks.length}\n` +
                                `Shared Networks: ${networks.filter((net) => net.shared).length}\n` +
                                `Edge Gateways: ${edgeGateways.length}\n` +
                                `Active Edge Gateways: ${edgeGateways.filter((edge) => edge.status === 'REALIZED').length}\n` +
                                `Running VMs: ${org.runningVMCount || 0}\n` +
                                `vApps: ${org.vappCount || 0}\n` +
                                `Catalogs: ${org.catalogCount || 0}`,
                        },
